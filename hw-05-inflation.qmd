...
a.  Look at the `inflation_wide` data frame and answer the following questions based on the output.
    -   How many rows does `inflation_wide` have and what does each row represent?
    -   How many columns does `inflation_wide` have and what does each column represent?
...
-   store the result in a new data frame called `inflation_wide_ratios`.

Then, in two separate pipelines,

-   arrange `inflation_wide_ratios` in increasing order of `inf_ratio` and
-   arrange `inflation_wide_ratios` in decreasing order of `inf_ratio`.
...
Reshape (pivot) `inflation_wide` such that each row represents a country/year combination. Then, display the resulting data frame and state how many rows and columns it has and write a sentence stating the number of rows and columns as well as the names of the columns of the resulting data frame.
...
-   The resulting data frame must be saved as something other than `inflation_wide` so you (1) can refer to this data frame later in your analysis and (2) do not overwrite `inflation_wide`. Use a short but informative name.

::: callout-warning
Subsequent questions may require the use of the pivoted data frame from Question 4.
:::
...
For this question you will use the pivoted version of the `inflation_wide` dataset that you created in Question 4.
