...
The dataset for this assignment can be found as a csv file in the `~/shared/stat141/data` folder on our server.
You can read it in using the following.

```{r load-data, message=FALSE, eval=TRUE}
plastic_waste <- read_csv("~/shared/stat141/data/plastic-waste.csv")

# Or we could read it straight from the internet through a URL.
# This should work on your laptop.
#plastic_waste <- read_csv("https://github.com/tidyverse/datascience-box/raw/refs/heads/main/course-materials/lab-instructions/lab-02/data/plastic-waste.csv")
```
...
-   Create a new code chunk below and use R to evaluate `355/113`.
...
1.  Modify the code below to redefine the variable `plastic_waste` to include only those rows where `plastic_waste_per_cap` is less than or equal to 1.
...
2.  Visualize the relationship between plastic waste per capita and mismanaged plastic waste per capita using a scatterplot. Describe the relationship. **Remember:** We use `geom_point()` to make scatterplots.
3.  Colour the points in the scatterplot by continent. Does there seem to be any clear distinctions between continents with respect to how plastic waste per capita and mismanaged plastic waste per capita are associated?
...
1.  Interpret the plot below in context of the data. Challenge yourself to write the code just by looking at the plot.
