...
```{r}
filter(bechdel90_13, roi <= 5 & 1994 <= year) %>%
  group_by(year, binary) %>%
  summarize(roi = median(roi)) %>%
  ggplot(mapping = aes(x = year, y = roi, color = binary)) +
  facet_wrap(~binary) +
  geom_point(alpha = 0.4) +
  geom_line() +
  #geom_smooth() +
  labs(title = "Median return on movie investment over time",
       y = "ROI (return on investment)", color = "Bechdel test")
```
